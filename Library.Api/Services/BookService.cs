...
public class BookService : IBookService
{
    ...
    public async Task<Book?> GetByIsbnAsync(string isbn)
    {
        ...
        return connection.QuerySingleOrDefault<Book>("SELECT * FROM Books WHERE Isbn = @Isbn LIMIT 1",
            new { Isbn = isbn });
    }
    ...
    public async Task<IEnumerable<Book>> SearchByTitleAsync(string searchTerm)
    {
        ...
        return await connection.QueryAsync<Book>("SELECT * FROM Books WHERE Title LIKE '%' || @SearchTerm || '%'", new
        {
    ...
    public async Task<bool> DeleteAsync(string isbn)
    {
        ...
        var result = await connection.ExecuteAsync(@"DELETE FROM Books WHERE Isbn = @Isbn ",
            new {Isbn = isbn});
