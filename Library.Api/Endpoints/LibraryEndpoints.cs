...
public class LibraryEndpoints : IEndpoints
{
    ...
    {
        app.MapPost($"{BaseRoute}", CreateBookAsync).WithName("CreateBook")
            .Accepts<Book>(ContentType)
            .Produces<Book>(201)
            .Produces<IEnumerable<ValidationFailure>>(400)
            .WithTags(Tag);

        app.MapGet($"{BaseRoute}", SearchBookByIsbn).WithName("GetBooks")
        ...
        app.MapGet("status", RenderHtmlAsync).RequireCors("AnyOrigin").ExcludeFromDescription(); //;
    }
    ...
    private static async Task<IResult> SearchBookByIsbn(IBookService bookService, string? searchTerm)
    {
    ...
    private static async Task<IResult> DeleteBookAsync(string isbn, IBookService bookService)
    {
        ...
        return book ? Results.Ok(book) : Results.NotFound();
    }

    private static async Task<IResult> RenderHtmlAsync()
    {
    ...
    <html>
        ...
        <body>
            <h1>Status</h1>
            <p>This server is working fine. Bye bye!</p>
        </body>
    </html>
    ");
    }
